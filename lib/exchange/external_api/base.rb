    ...
    class Base
      ...
      def rate from, to, opts={}
        rate = cache.cached(self.class, opts.merge(:key_for => [from, to])) do
          ...
          test_for_rates_and_raise_if_nil from, to, opts[:at]
      ...
      def test_for_rates_and_raise_if_nil rate_from, rate_to, time=nil
        unless rates[rate_from] && rates[rate_to]
          raise NoRateError.new("No rates where found for #{rate_from} to #{rate_to} #{'at ' + time.to_s if time}")
        end
      end
