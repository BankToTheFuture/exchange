...
module Exchange
  module ExternalAPI
    ...
    class Call < Base
      ...
      # @param resource The url of the API to call or data when options[:format] == :data
      ...
      #
      # @example get data - dont call api
      #   Exchange::ExternalAPI::Call.new(data, format: :data)
      #
      #
      def initialize(resource, options = {}, &block)
        data = resource if options[:format] == :data
        data ||= load_url(resource, options[:retries] || config.retries, options[:retry_with])
